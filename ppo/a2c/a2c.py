...
from ppo.common import set_global_seeds, explained_variance
from ppo.common.models import get_network_builder
from ppo.common.policies import PolicyWithValue

from ppo.a2c.utils import InverseLinearTimeDecay
from ppo.a2c.runner import Runner
from ppo.ppo2.ppo2 import safemean
...
def learn(
    ...
    **network_kwargs):
    ...
    network:            policy network architecture. Either string (mlp, lstm, lnlstm, cnn_lstm, cnn, cnn_small, conv_only - see ppo.common/models.py for full list)
                        ...
                        See ppo.common/policies.py/lstm for more details on using recurrent nets in policies


    env:                RL environment. Should implement interface similar to VecEnv (ppo.common/vec_env) or be wrapped with DummyVecEnv (ppo.common/vec_env/dummy_vec_env.py)
